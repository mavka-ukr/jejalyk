...
NodeCompilationResult* compile_take_pak_node(
  ...
  const auto module_name_result =
      options->get_remote_module_name(take_pak_node->name, options);
  ...
  const auto module_path_result =
      options->get_remote_module_path(take_pak_node->name, options);
  ...
  const auto module_variable_name =
      take_pak_node->as.empty() ? module_name : take_pak_node->as;
  ...
  const auto init_module_code = "await init_" + temp_module_name + "();\n" +
                                ...
                                temp_module_name;
  if (state->has_module(module_path)) {
    node_compilation_result->result = init_module_code;
    return node_compilation_result;
  }
  state->set_module(module_path, "");
  ...
  const auto module_code_result = options->get_remote_module_code(
      take_pak_node->name, scope_compilation_options);
  ...
  if (take_pak_node->as.empty()) {
    scope->assign(module_name, nullptr);
  } else {
    scope->assign(take_pak_node->as, nullptr);
  }
  ...
  if (module_parser_result->error) {
    ...
    node_compilation_result->error->column =
        module_parser_result->error->column;
    node_compilation_result->error->message =
        module_parser_result->error->message;
    node_compilation_result->error->message = module_name_result->error;
    return node_compilation_result;
  }
