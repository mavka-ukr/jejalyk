...
NodeCompilationResult* compile_eval_node(mavka::ast::EvalNode* eval_node,
  ...
  if (!options->allow_js) {
    ...
    node_compilation_result->error->message =
        "JS-розширення вимкнені. Додайте опцію \"--розширення=1\", щоб "
        "увімкнути.";
    return node_compilation_result;
  }
  if (tools:: instanceof <mavka::ast::StringNode>(eval_node->value)) {
    const auto string_value =
        dynamic_cast<mavka::ast::StringNode*>(eval_node->value)->value;
    node_compilation_result->result =
        "/* --- js --- */\n" + string_value + "\n/* --- /js --- */";
  } else {
    const auto eval_value =
        compile_node(eval_node->value, scope, options, state);
  }
        return node_compilation_result;
    }
}
