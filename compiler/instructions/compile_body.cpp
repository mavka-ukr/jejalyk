...
NodeCompilationResult* compile_body(
  ...
  for (const auto node : body) {
    if (i == body.size() - 1 && handle_return) {
      if (!tools:: instanceof
          <mavka::ast::ReturnNode>(node) && !tools:: instanceof
          <mavka::ast::EachNode>(node) && !tools:: instanceof
          <mavka::ast::WhileNode>(node) && !tools:: instanceof
          <mavka::ast::IfNode>(node) && !tools:: instanceof
          <mavka::ast::BreakNode>(node) && !tools:: instanceof
          <mavka::ast::ContinueNode>(node)) {
        const auto node_compilation_result =
            compile_node(node, scope, options_clone, state);
    ...
    const auto item_node_compilation_result =
        compile_node(node, scope, options_clone, state);
  ...
  if (!vars.empty()) {
    compiled_body.insert(compiled_body.begin(),
                         "var " + tools::implode(vars, ","));
  }
