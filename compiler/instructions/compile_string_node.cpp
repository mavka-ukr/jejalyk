  ...
  NodeCompilationResult* compile_string_node(
    ...
    for (int i = 0; i < string_node->value.length(); ++i) {
      if (!interpolation &&
        ...
        node_compilation_result->error->message =
            "Числа не підтримуються в рядках.";
    ...
    for (const auto part : parts) {
      if (tools:: instanceof <mavka::ast::StringNode>(part)) {
        ...
        const auto part_compilation_result =
            compile_node(part, scope, options, state);
        if (part_compilation_result->error) {
          node_compilation_result->error = part_compilation_result->error;
          return node_compilation_result;
        }
        const auto diName = state->put_debug(
            options->current_module_path, part->start_line, part->start_column);
        string_parts.push_back("мВикл(м_текст.чародія_викликати, [" +
                               part_compilation_result->result + "]," + diName +
                               ")");
      }
    }

    node_compilation_result->subject = create_text_instance_subject(scope);
