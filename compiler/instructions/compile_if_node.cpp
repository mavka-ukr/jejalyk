...
NodeCompilationResult* compile_if_node(const mavka::ast::IfNode* if_node,
  ...
  const auto condition =
      compile_node(if_node->condition, scope, options, state);
  if (condition->error) {
  ...
  const auto body = compile_body(if_node->body, if_scope, options, state, true);
  if (body->error) {
  ...
  if (!if_node->else_body.empty()) {
    ...
    const auto else_body =
        compile_body(if_node->else_body, else_if_scope, options, state, true);
    if (else_body->error) {
    ...
    node_compilation_result->result =
        "if(" + condition->result + ")" + body->result + "";
  }
  return node_compilation_result;
}
}  // namespace jejalyk
