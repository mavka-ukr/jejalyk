...
NodeCompilationResult* compile_test_node(const mavka::ast::TestNode* test_node,
  ...
  const auto left = compile_node(test_node->left, scope, options, state);
  if (left->error) {
    node_compilation_result->error = left->error;
    return node_compilation_result;
  }
  const auto right = compile_node(test_node->right, scope, options, state);
  if (right->error) {
  ...
  if (test_node->op == "і") {
    ...
    node_compilation_result->error->message =
        "unsupported test operation: " + test_node->op;
  }
  return node_compilation_result;
}
}
