...
namespace jejalyk {
  NodeCompilationResult* compile_anon_diia_node(
    ...
    const auto params_compilation_result =
        compile_params(anon_diia_node->params, anon_diia_scope, options, state);
    ...
    const auto body =
        compile_diia_body(anon_diia_node->body, anon_diia_node->params,
                          anon_diia_scope, options, state, true);
    if (body->error) {
    ...
    node_compilation_result->result =
        ...
        (anon_diia_node->async ? "async " : "") + "function(args)" +
        body->result + "," + diName + ")";
    return node_compilation_result;
  }
}  // namespace jejalyk
