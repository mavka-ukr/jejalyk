...
NodeCompilationResult* compile_function_node(
  ...
  const auto diName =
      state->put_debug(options->current_module_path, function_node->start_line,
  ...
  const auto params_compilation_result =
      compile_params(function_node->params, function_scope, options, state);
  ...
  const auto body =
      compile_diia_body(function_node->body, function_node->params,
                        function_scope, options, state, true);
  if (body->error) {
    node_compilation_result->error = body->error;
    return node_compilation_result;
  }
  node_compilation_result->result =
      MAVKA_DIIA + "(null," + compiled_params + "," +
      (function_node->async ? "async " : "") + "function(args)" + body->result +
      "," + diName + ")";
  return node_compilation_result;
}
}  // namespace jejalyk
